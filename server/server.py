...
@app.post("/start-agent/{deployment_id}")
async def start_agent_deployment_id(deployment_id: str):
    """Start a Gemini agent pipeline with a specific deployment ID."""

    try:
        agent_file = "bot-gemini"
        proc = subprocess.Popen(
            [f"python3 -m {agent_file} --deployment_id {deployment_id}"],
            shell=True,
            bufsize=1,
            cwd=os.path.dirname(os.path.abspath(__file__)),
        )
        bot_procs[proc.pid] = (proc, f"agent-{deployment_id}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to start agent: {e}")

    return JSONResponse({"agent_pid": proc.pid, "deployment_id": deployment_id, "status": "started"})
