...
class ChatbotClient {
  ...
  setupDOMElements() {
    ...
    this.agentIdInput = document.getElementById('agent-id-input');
    this.startAgentBtn = document.getElementById('start-agent-btn');
  ...
  setupEventListeners() {
    ...
    this.startAgentBtn.addEventListener('click', () => {
  ...
  fetch(`/start-agent/${agentId}`, {
    method: "POST",
  })
    .then((response) => {
      if (response.ok) {
        this.log(`Agent started with ID: ${agentId}`);
      } else {
        this.log("Failed to start agent.");
      }
    })
    .catch((error) => {
  ...
  async connect() {
    try {
      ...
      await this.pcClient.connect({
        ...
        endpoint: 'http://localhost:8080/connect',
        timeout: 25000,
      });
